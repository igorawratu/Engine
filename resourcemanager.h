...
public:
     ...
     * with SHADER_FILE being from file, and SHADER_RAW being actual shader code.
     ...
     */
    Shader* createShader(const std::string& vs, const std::string& fs, ShaderDataType data_type);

    /**
     * @brief Gets pointer to the shader with the specified \p id, or nullptr of no shader with that \p id exists.
     * @param id ID of the shader to return
     * @return Observer pointer to shader
     */
